...
window.onload = function () {
  ...
  const sketch = (p) => {
    ...
    function drawRayShape(z, centerX, centerY) {
      // Draw radial rays pattern
      for (let l = 0; l < spokes; l += 1) {
        const angle = p.TWO_PI * (l / spokes);

        p.beginShape();
        // Create wavy ray path
        const points = [];
        const segments = 9;
        for (let i = 0; i <= segments; i += 1) {
          const t = i / segments;
          const dist = distribution * (0.2 + t * 0.9);
          const waveOffset = (i % 2 === 0 ? 1 : -1) * wavyness * ((z + 1) / swirly);

          const x = centerX + Math.cos(angle) * dist + Math.cos(angle + p.HALF_PI) * waveOffset;
          const y = centerY + Math.sin(angle) * dist + Math.sin(angle + p.HALF_PI) * waveOffset;
          points.push({ x, y });
        }

        // Draw the ray with offset width
        const offsetDist = minOffset;
        for (let i = 0; i < points.length; i += 1) {
          const pt = points[i];
          const perpAngle = angle + p.HALF_PI;
          const x1 = pt.x + Math.cos(perpAngle) * offsetDist;
          const y1 = pt.y + Math.sin(perpAngle) * offsetDist;
          p.vertex(x1, y1);
        }
        for (let i = points.length - 1; i >= 0; i -= 1) {
          const pt = points[i];
          const perpAngle = angle - p.HALF_PI;
          const x2 = pt.x + Math.cos(perpAngle) * offsetDist;
          const y2 = pt.y + Math.sin(perpAngle) * offsetDist;
          p.vertex(x2, y2);
        }
        p.endShape(p.CLOSE);
        ...
        for (let n = dripstart; n < dripRadius; n += 1) {
          if (noise.get(l, n, z) > dripfrequency) {
            const t = n / segments;
            const dist = distribution * (0.2 + t * 0.7);
            const dripX = centerX + Math.cos(angle) * dist;
            const dripY = centerY + Math.sin(angle) * dist;
    ...
    function drawFrame(z) {
      ...
      const innerW = wide - framewidth * 2;
      const innerH = high - framewidth * 2;

      // Draw frame border (between outer and inner)
      p.fill(colors[z].Hex);
      p.beginShape();
      p.vertex(0, 0);
      p.vertex(wide, 0);
      ...
      p.vertex(innerX, innerY + innerH);
      p.vertex(innerX + innerW, innerY + innerH);
      p.vertex(innerX + innerW, innerY);
      p.endContour();
      p.endShape(p.CLOSE);
    }

    function drawCutMarks(z) {
      if (z < stacks - 1 && z !== 0) {
        p.fill(colors[z].Hex);
        p.noStroke();
        for (let etch = 0; etch < stacks - z; etch += 1) {
          p.circle(50 + etch * 10, 25, 4);
        }
      }
    }

    function drawHanger(z) {
      if (z < stacks - 2 && scale > 0) {
        ...
        // Left hanger
        p.rect(framewidth / 2 - r, framewidth - r * 1.5, r * 2, r * 3, r);

        // Right hanger
        p.rect(wide - framewidth / 2 - r, framewidth - r * 1.5, r * 2, r * 3, r);

        // Top center hanger
        p.rect(wide / 2 - r * 2, framewidth / 2 - r, r * 4, r * 2, r);
      }
    }
    ...
    function drawArt() {
      ...
      for (let z = 0; z < stacks; z += 1) {
        ...
        // Draw rays pattern (skip for last layer)
        if (z < stacks - 1) {
          p.noStroke();
          p.fill(colors[z].Hex);
          drawRayShape(z, centerX, centerY);
        }

        // Draw cut marks and hangers
        drawCutMarks(z);
        drawHanger(z);
